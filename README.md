...
**出力先は既定で PC 再生**。「この季節を体験する」を押すと別タブで `display.html?source=pc`（表示専用ページ）が開き、操作画面とは `BroadcastChannel` / `localStorage` で即時同期する（KVは使わない）。再生タブは最初に1回クリックして音声を許可する。スマホが接続済みなら体験開始時に自動でスマホ出力に切り替わり、体験画面ヘッダーの「出力: 🖥 PC / 📱 スマホ」でいつでも手動切替できる（「🖥 PC」を押すと再生タブを開く／前面に出す）。スマホ出力のときだけシーンを KV に送り、PC タブには停止を送るので二重再生にならない。スマホなしでも PC だけでデモが成立する。
