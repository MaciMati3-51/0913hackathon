...
async function requestScene(fetcher, apiKey, model, system, userContent, base, timeout) {
  ...
  if (result.status !== 'completed' || typeof result.output_text !== 'string') {
    throw new Error(`OpenAI returned no completed scene (status: ${result.status})`);
  }
  return normalizeScene(JSON.parse(result.output_text), base);
}

async function withFallback(env, timeout, mock, run, options) {
  if (!env?.OPENAI_API_KEY) return { scene: mock(), source: 'mock' };
  const fetcher = options.fetcher ?? fetch;
  try {
    return { scene: await run(fetcher, env.OPENAI_API_KEY, env?.LLM_MODEL || DEFAULT_MODEL), source: 'llm' };
  } catch (error) {
...
export function generateScene(text, env, options = {}) {
  ...
  return withFallback(env, GENERATE_TIMEOUT_MS, mock, (fetcher, apiKey, model) =>
    requestScene(fetcher, apiKey, model, SYSTEM_GENERATE, `体験したいシーン: ${text}`, mock(), GENERATE_TIMEOUT_MS), options);
}

export function adjustScene(scene, text, env, options = {}) {
  const mock = () => mockAdjustScene(scene, text);
  const userContent = `現在のシーン:\n${JSON.stringify(scene)}\n\n体験中の発話: ${text}`;
  return withFallback(env, ADJUST_TIMEOUT_MS, mock, (fetcher, apiKey, model) =>
    requestScene(fetcher, apiKey, model, SYSTEM_ADJUST, userContent, scene, ADJUST_TIMEOUT_MS), options);
}
